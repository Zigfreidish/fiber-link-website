...
function processImage(img) {
  ...
  const scale =
    w > h
      ? w > MAX_SIZE ? MAX_SIZE / w : w < MIN_SIZE ? MIN_SIZE / w : 1
      : h > MAX_SIZE ? MAX_SIZE / h : h < MIN_SIZE ? MIN_SIZE / h : 1;
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const i = y * w + x;
      if (!shape[i]) continue;
      if (!x || x === w - 1 || !y || y === h - 1 || !shape[i-1] || !shape[i+1] || !shape[i-w] || !shape[i+w])
        boundary[i] = 1;
    }
  }

  const u = new Float32Array(size);
  const C = 0.01, omega = 1.85;
  for (let iter = 0; iter < 200; iter++) {
    for (let y = 1; y < h - 1; y++) {
      for (let x = 1; x < w - 1; x++) {
        const i = y * w + x;
        if (!shape[i] || boundary[i]) continue;
        const sum = (shape[i+1] ? u[i+1] : 0) + (shape[i-1] ? u[i-1] : 0) +
                    (shape[i+w] ? u[i+w] : 0) + (shape[i-w] ? u[i-w] : 0);
...
export default function MetallicPaint({
  ...
  useEffect(() => {
    ...
    const canvas = canvasRef.current;
    const gl = glRef.current;
    const side = Math.round(1000 * devicePixelRatio);
    canvas.width = side;
    canvas.height = side;
